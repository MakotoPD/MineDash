...
ICON_SRC="${ROOT_DIR}/src-tauri/icons/voidlink.icon"
OUT_DIR="${ROOT_DIR}/src-tauri/icons/.compiled-icon"
ASSETS_CAR="${ROOT_DIR}/src-tauri/icons/Assets.car"

if [[ -f "${ASSETS_CAR}" && ( "${CI:-}" == "true" || "${GITHUB_ACTIONS:-}" == "true" ) ]]; then
  echo "VoidLink icon already compiled; using ${ASSETS_CAR}"
  exit 0
fi
...
echo "VoidLink icon compiled: ${ASSETS_CAR}"
